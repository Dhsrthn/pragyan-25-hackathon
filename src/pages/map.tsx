...
function Map() {
  ...
  const handleRelayout = (event) => {
    if (event["mapbox.center"] || event["mapbox.zoom"]) {
      console.log(event["mapbox.center"], mapState.center);
      updateMapState({
        zoom: event["mapbox.zoom"] || mapState.zoom,
        center: JSON.parse(
          JSON.stringify(event["mapbox.center"] || mapState.center)
        ),
      });
    }
  };
  ...
  useEffect(() => {
    ...
    setPopulationData((prevData) =>
      prevData.map((row) =>
        inaccessibleNodes.has(row.GEO_ID) ? { ...row, COLOR_STATE: 2 } : row
      )
    );
  }, [inaccessibleNodes]);
  ...
  useEffect(() => {
    if (selectedNodes.start != null) {
      ...
      setPopulationData((prevData) =>
        prevData.map((row) =>
          row.GEO_ID === selectedNodes.start.GEO_ID
            ? { ...row, COLOR_STATE: 1 }
            : row
        )
      );
    ...
    if (selectedNodes.destination != null) {
      ...
      setPopulationData((prevData) =>
        prevData.map((row) =>
          row.GEO_ID === selectedNodes.destination.GEO_ID
            ? { ...row, COLOR_STATE: 1 }
            : row
        )
      );
