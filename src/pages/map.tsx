...
function Map() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch CSV data
                const csvResponse = await fetch("output3.csv");
                ...
                const rows = csvText
                    ...
                    .slice(1)
                    .map((row) => {
                        const [GEO_ID, STATE, COUNTY, NAME, LSAD, CENSUSAREA, COLOR_STATE] =
                            row.split(",");
                        return {
                            GEO_ID,
                            STATE,
                            ...
                            COLOR_STATE: parseInt(COLOR_STATE),
                        };
                    })
    ...
    useEffect(() => {
        if (pathData.size !== 0) {
            ...
            setPopulationData((prevData) =>
                prevData.map((row) =>
                    pathData.some((node) => node.id === row.GEO_ID)
                        ? { ...row, COLOR_STATE: 1 }
                        : row
                )
            );
    ...
    useEffect(() => {
        if (inaccessibleNodes.size === 0) {
            if (selectedNodes.start == null && selectedNodes.destination == null) {
                setVisibleColors(1);
            } else {
                setVisibleColors(2);
            }
        ...
        setPopulationData((prevData) =>
            prevData.map((row) =>
                inaccessibleNodes.has(row.GEO_ID)
                    ? { ...row, COLOR_STATE: 2 }
                    : row
            )
        );
    ...
    useEffect(() => {
        if (selectedNodes.start != null) {
            ...
            setPopulationData((prevData) =>
                prevData.map((row) =>
                    row.GEO_ID === selectedNodes.start.GEO_ID ? { ...row, COLOR_STATE: 1 } : row
                )
            );
        }
        if (selectedNodes.destination != null) {
            setVisibleColors(2);
            setPopulationData((prevData) =>
                prevData.map((row) =>
                    row.GEO_ID === selectedNodes.destination.GEO_ID ? { ...row, COLOR_STATE: 1 } : row
                )
            );
        }
    }, [selectedNodes])
    ...
    const handleBlockClick = async (geoId) => {
        if (mode === Mode.REMOVE) {
            if (selectedNodes.start == null || selectedNodes.destination == null) return
                        ...
                        <div className="p-2 bg-white -translate-x-1 -translate-y-1 flex flex-col gap-4 items-center h-full border">
                            ...
                            <div className="h-[80%] flex flex-col justify-around items-center">
                                <div className="bg-black h-fit w-fit">
                                    <button
                                        className={`px-5 py-2 ${mode == Mode.INFORMATION
                                ...
                                <div className="bg-black h-fit w-fit">
                                    <button
                                        className={`px-5 py-2 ${mode == Mode.REMOVE
                                            ? "bg-[#007a00] text-white -translate-x-2 -translate-y-2"
                                            : "-translate-x-1 -translate-y-1 border-2 border-[#007a00] bg-white text-[#007a00] transition-all hover:-translate-x-2 hover:-translate-y-2 hover:text-white hover:bg-[#007a00]"
                                            } hover:cursor-pointer`}
                                        onClick={() => {
                                ...
                                <div className="bg-black h-fit w-fit">
                                    <button
                                        className={`px-5 py-2 ${mode == Mode.ROAD
                                            ? "bg-[#007a00] text-white -translate-x-2 -translate-y-2"
                                            : "-translate-x-1 -translate-y-1 border-2 border-[#007a00] bg-white text-[#007a00] transition-all hover:-translate-x-2 hover:-translate-y-2 hover:text-white hover:bg-[#007a00]"
                                            } hover:cursor-pointer`}
                                        onClick={() => {
                            ...
                            <div className="flex flex-col justify-between mt-2">
                                <div className="w-1/2 p-2 border-b border-gray-300">
                                    {inaccessibleNodes && inaccessibleNodes.size > 0 ? (
                                        Array.from(inaccessibleNodes).map((ele, index) => {
                                            return (
                    ...
                    data={[
                        {
                            ...
                            z: populationData.map((row) => row.COLOR_STATE),
                            colorscale: visibleColors === 1 ? [[0, "#7cb9e8"], [1, "#7cb9e8"]] : visibleColors === 2 ? [[0, "#7cb9e8"], [0.5, "#7cb9e8"], [0.5, "green"], [1, "#007a00"]] : [[0, "#7cb9e8"], [0.33, "#7cb9e8"], [0.33, "#007a00"], [0.66, "#007a00"], [0.66, "#ff033e"], [1, "#ff033e"]],
